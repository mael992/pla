            ...
            <table class="table table-hover mb-0" id="incidentsTable">
                ...
                @forelse($incidents as $incident)
                    <tr data-id="{{ $incident->id_incident }}">
                        <td class="fw-semibold text-muted">{{ $incident->id_incident }}</td>
                        <td>
                            {{ $incident->date_emis
...
async function pollIncidents() {
    try {
        ...
        if (data.nouveaux && data.nouveaux.length > 0) {
            pendingIncidents = data.nouveaux;
            lastKnownId = data.last_id;

            chargerNouveaux(); // AUTO INSERT
        }
...
function chargerNouveaux() {
    ...
    [...pendingIncidents].reverse().forEach(inc => {

        // IMPORTANT FIX id_incident
        if (tbody.querySelector(`tr[data-id="${inc.id_incident}"]`)) return;
...
function buildRow(inc) {
    ...
    tr.dataset.id = inc.id_incident;

    tr.innerHTML = `
        <td class="fw-semibold text-muted">${inc.id_incident}</td>
        <td>${inc.date_emis ?? '—'}</td>
        <td>${inc.zone ?? '—'}</td>
